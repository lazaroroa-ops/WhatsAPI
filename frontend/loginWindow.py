...
class LoginWindow():
	def __init__(self, stdscr):
		...
		self.window.addstr(2, int(self.width / 2 - len("Welcome back!") / 2), "Welcome back!")
	...
	def change_focus(self, index):
		...
		self._draw_styled_text("Log in", 13, int(self.width / 2 - len("Log in") / 2), index == 2)
		self._draw_styled_text("Not registered?", 15, int(self.width / 2 - len("Not registered?") / 2), index == 3)
